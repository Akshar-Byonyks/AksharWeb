...
- **TWO FLAGS. ONE IS NOW CLOSED, ONE IS STILL OPEN — they were never the same kind of problem:**
  1. **STILL OPEN. The subject is not Indian.** PRODUCT.md is emphatic that content is India-specific and that material which does not transfer gets replaced rather than kept for volume. A white Western man is a visible mismatch on an India-market site. Replace with India-shot photography when it exists; until then this is the only real image of the device in a home. **This flag is editorial, not legal, and nothing about the sign-off below touches it.** It is closed by commissioning a photograph, not by anyone approving one.
  2. **CLOSED 10 SEP 2026. It depicts a person with a medical device in a therapy context.** The Drugs and Magic Remedies (Objectionable Advertisements) Act 1954 constrains this, and this repo has previously treated that constraint strictly enough to patch a bystander's reflection out of a monitor. Using it was an explicit client decision made on 28 Aug 2026 after the risk was raised, and it was carried since then as a launch gate (spec §14.4) on the stated ground that this is a legal question and not a designer's judgement. **The client confirmed on 10 Sep 2026 that the sign-off has been given.** The gate is retired on that confirmation. The record here does not name who gave it or on what date they gave it — if that is ever needed, it needs adding rather than reconstructing.
...
- **ORIGIN CONFIRMED: IT IS A REAL PHOTOGRAPH.** The client confirmed this on 10 Sep 2026, and the gate that stood here is closed.

  What stood here, kept because the reasoning is worth having on the record rather than deleted as a false alarm: several properties of the file were consistent with an AI-generated or heavily AI-retouched likeness rather than a camera photograph. It arrived as a **PNG** where a camera or a phone produces JPEG; it carried **no EXIF whatsoever** — no camera, no date, no exposure; it is **72dpi and near-square**; and the rendering of the skin, the beard edges and the background falloff had a smoothness characteristic of generated imagery.

  **None of those was proof, and the file was flagged rather than judged.** Every one of them is also what happens to a real photograph that has been exported through an editor, saved off LinkedIn or screenshotted — all of which strip EXIF and can produce a square PNG. That is exactly why the question went to the client instead of being decided here, and the answer is that the signals were artefacts of how the file travelled, not of how the image was made.

  **The lesson to carry, not the suspicion:** metadata absence is evidence about a file's history, never about its subject. A stripped EXIF block tells you a picture has been through software. It does not tell you a person was generated.
- **Why it mattered here specifically.** CLAUDE.md's standing rule is "never caption a render as a photograph," and this is a named, board-certified physician on the leadership page of a medical-device company — a generated likeness presented as a portrait would have been a different kind of claim from a stylised illustration elsewhere on the site. The `portraitAlt` in `leadership.ts` was written to describe what is in the frame **without asserting it is a photograph**, which was the honest position while the question was open. It needs no change now that the answer is "photograph": describing the frame is accurate either way, and nothing on the site captions it as a render.
