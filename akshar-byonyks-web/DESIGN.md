...
### Navigation
- **Style:** flat text links, `text-sm font-medium`, rounded hover background (`surface-2`). Mobile and tablet: full-screen native `<dialog>` drawer, the system's one client-interactive nav pattern.
- **The header has two forms, and the boundary is `lg` (64rem).** They are different objects, not one object rescaled, and that is the whole of the split.
  - **At `lg` and up, a floating capsule** (`components/ui/animated-nav.tsx`, deviations §20): a pill in a transparent header strip, seven links plus the language switch and the CTA, collapsing to a 48px dot on a committed downward scroll and returning on an upward one. It is the site's one piece of chrome that behaves like an object rather than a surface.
  - **Below `lg`, a real bar** (10 Sep 2026, deviations §24): full-bleed, opaque `background/95` over `backdrop-blur`, a `line` rule along the bottom, `min-h-14`. Emblem and the "Akshar Byonyks" wordmark on the left, `हिन्दी` and the drawer trigger on the right — controls on the edges, where thumbs are. **It does not collapse.**
- **Why they are not the same object.** A pill floating in a transparent strip needs margin either side of the text column to float over. A phone has none, so the capsule was scrolling across the middle of the measure and covering the prose — measured at up to 47px of running text on eleven of fourteen routes. And the collapse only pays for itself where it hides something: below `lg` the seven links are already in the drawer and the bar holds three controls, so it saved no space and cost a tap. **The rule this leaves behind: sticky chrome that does not paint an opaque, full-bleed surface must not sit over a full-width text column.**
- **The wordmark is set beside the emblem below `lg` only,** because that is where the bar has room and where nothing else on screen says whose site this is. The emblem's `alt` is empty and the wordmark carries the home link's accessible name at every width — it is merely sighted below `lg` and `sr-only` at `lg`, so the name is announced once, never twice.
- **Touch targets are 44px below `lg`,** including the drawer's child links and both icon buttons. Standalone links elsewhere on the site reach the same floor through the `tap-target` utility, which grows the hit area and gives the space back with a negative block margin so no layout moves. Links inside running prose are exempt (WCAG 2.5.8) and are left alone.
- **The drawer carries everything the bar carries,** because it covers the bar: Home on the wordmark, every section and child, the CTA, and the language switch. A control that only exists in the bar does not exist while the menu is open.
